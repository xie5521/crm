		...
		$(function(){
			...
			$("#deleteClueBtn").click(function () {
				...
				if(window.confirm("确定删除吗？")){
					var ids = "";
					$.each(clueIds, function (index, obj){	//id=xxxx&id=xxxx&id=xxxxx
						ids += "id="+ this.value + "&";
					})
			...
			$("#editClueBtn").click(function () {
				//收集参数
				//获取列表中被选中的checkbox
				var clueIds = $("#tBody input[type='checkbox']:checked");
				if(clueIds.size() == 0){
					alert("请选择要修改的市场活动。")
					return;
				}
				...
				$.ajax({
					...
					success:function (data) {
						// 给修改界面的模态窗口的标签赋值
						$("#edit-id").val(data.id);
						$("#edit-clueOwner").val(data.owner); // 通过设置value值（owner的id）循环遍历出来owner
