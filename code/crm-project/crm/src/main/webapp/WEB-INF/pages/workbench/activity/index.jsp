...
<script type="text/javascript">
	...
	$(function(){
		...
		//当市场活动主页加载完成，查询所有数据的第一页以及所有数据的总条数
		queryActivityByConditionForPage(1, 10);

		//给“查询”按钮添加单击事件
		$("#queryActivityBtn").click(function (){
			//"查询“所有符合条件数据的第一页以及总条数
			queryActivityByConditionForPage(1, $("#demo_pag1").bs_pagination('getOption', 'rowsPerPage'));
		})

		//给“全选”按钮添加单击事件
		$("#checkAll").click(function (){
			//如果“全选”按钮是选中状态，则列表中所以checkbox都选中
			/*if (this.checked == true) {9+hecked", true);
			}else{
		...
		$("#deleteActivityBtn").click(function (){
			...
			var ids = "";
			$.each(chekkedIds, function (index, obj){	//id=xxxx&id=xxxx&id=xxxxx
				ids += "id="+ this.value + "&";
			})
        ...
        $("#exportActivityAllBtn").click(function (){
			...
			$("#exportActivityXzBtn").click(function () {
				...
				if(window.confirm("您确定要导出这"+checkedIds.size()+"条活动吗？")){
					...
					window.location.href="workbench/activity/exportXzActivitys.do?ids="+ids;//注意传参格式
				}
			});
		...
		$("#importActivityBtn").click(function (){
			...
			//发送请求
			//FormData是ajax提供的一个接口，可以模拟键值对向后台提交参数
			//不但可以提交文本文件，还可以提交二进制数据
			...
			$.ajax({
				...
				type:'post',
				dataType:'json',
				processData:false,		//设置ajax向后台提交参数之前，是否把参数统一转换成字符串，默认为true---是，false---不是
				contentType:false,		//设置ajax向后台提交参数之前，是否把参数统一按yrlencoded编码，默认为true---是，false---不是
				success:function (data){
					if(data.code == "1"){
						//提示成功导入多少条记录
